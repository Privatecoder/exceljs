...
class RowXform extends BaseXform {
  ...
  setCurrentAddress(node) {
    switch (node.name) {
      case 'row': {
        if (this.currentAddress.row) {
          this.currentAddress.row += 1;
        } else {
          this.currentAddress.row = 1;
        }
        this.currentAddress.col = null;
        break;
      }
      case 'c': {
        if (this.currentAddress.col) {
          this.currentAddress.col += 1;
        } else {
          this.currentAddress.col = 1;
        }
        break;
  ...
  parseOpen(node) {
    ...
    this.setCurrentAddress(node);

    if (!node.attributes.r) {
      if (node.name === 'row') {
        node.attributes.r = String(this.currentAddress.row);
      } else if (node.name === 'c') {
        node.attributes.r = colCache.encodeAddress(this.currentAddress.row, this.currentAddress.col);
      }
    }
